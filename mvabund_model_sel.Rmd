...
water.pca = rda(na.omit(data_waterqual), scale=T) #all are in same units, no need to scale
#summary(water.pca)
...
```{r}
glm1 = manyglm(plankton_mv ~ data_nona$Site * 
                 as.factor(data_nona$Depth_ctgry) + 
                 data_nona$DO + 
                 data_nona$temp_C + 
                 data_nona$salinity +
...
```{r}
glm2 = manyglm(plankton_mv ~ data_nona$Site * 
                 as.factor(data_nona$Depth_ctgry) + 
                 data_nona$DO + 
                 data_nona$temp_C + 
                 data_nona$salinity +
...
```{r}
glm3 = manyglm(plankton_mv ~ data_nona$Site * 
                 as.factor(data_nona$Depth_ctgry) + 
                 data_nona$DO + 
                 data_nona$temp_C + 
                 data_nona$salinity +
...
drop_test_3
```
Again. since we still have a lot of terms, we will go ahead and toss out the interaction term with the next lowest AIC: the interaction between salinity and chlorophyll.

```{r}
glm4 = manyglm(plankton_mv ~ data_nona$Site * 
                 as.factor(data_nona$Depth_ctgry) + 
                 data_nona$DO + 
                 data_nona$temp_C + 
                 data_nona$salinity +
...
drop_test_4
```
At this point, no single (dropped) term can make the model significantly better. However, we will keep dropping the lowest AIC value just to see if we get to another point when the model improves again.

```{r}
glm5 = manyglm(plankton_mv ~ data_nona$Site * 
                 as.factor(data_nona$Depth_ctgry) + 
                 data_nona$DO + 
                 data_nona$temp_C + 
                 data_nona$salinity +
...
drop_test_5
```
...It only got worse... So we will stick with glm4, which kept the term dropped in glm5.
...
```{r}
#model2 = anova.manyglm(glm5,p.uni = "adjusted") #took 22 min
